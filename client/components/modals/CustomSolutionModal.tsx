...
import { Plus, FileText, Settings, AlertCircle, Shield, Users, Database } from "lucide-react";
...
export default function CustomSolutionModal({ isOpen, onClose, onSuccess }: CustomSolutionModalProps) {
  ...
  const complianceOptions = [
    { id: "gdpr", label: "GDPR (General Data Protection Regulation)", icon: Shield },
    { id: "ccpa", label: "CCPA (California Consumer Privacy Act)", icon: Shield },
    { id: "hipaa", label: "HIPAA (Health Insurance Portability)", icon: Users },
    { id: "sox", label: "SOX (Sarbanes-Oxley Act)", icon: FileText },
    { id: "iso27001", label: "ISO 27001 (Information Security)", icon: Database },
  ...
  const validateForm = () => {
    ...
    if (formData.complianceFramework.length === 0) newErrors.complianceFramework = "Select at least one compliance framework";
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Design a tailored contract analysis solution for your specific needs and compliance requirements.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-base font-medium text-[#271D1D]">Basic Information</h3>
            ...
            <div className="space-y-2">
              ...
              {errors.contractType && <p className="text-red-500 text-xs">{errors.contractType}</p>}
            </div>
          </div>

          {/* Compliance Frameworks */}
          <div className="space-y-4">
            <h3 className="text-base font-medium text-[#271D1D]">Compliance Frameworks</h3>
            <p className="text-sm text-gray-600">Select the compliance standards this solution should check against:</p>
            
            <div className="grid grid-cols-1 gap-3">
              {complianceOptions.map((option) => {
                const IconComponent = option.icon;
                return (
                  <div key={option.id} className="flex items-center space-x-3 p-3 rounded-lg border border-gray-200 hover:bg-gray-50">
                    <Checkbox
                      id={option.id}
            ...
            {errors.complianceFramework && <p className="text-red-500 text-xs">{errors.complianceFramework}</p>}
          </div>

          {/* Analysis Configuration */}
          <div className="space-y-4">
            <h3 className="text-base font-medium text-[#271D1D]">Analysis Configuration</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.riskLevel} onValueChange={(value) => handleInputChange("riskLevel", value)}>
                  ...
                  <SelectContent>
                    <SelectItem value="conservative">Conservative (Strict)</SelectItem>
          ...
          {/* Custom Rules */}
          <div className="space-y-4">
            <h3 className="text-base font-medium text-[#271D1D]">Custom Rules (Optional)</h3>
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                ...
                These rules will be processed by our AI engine to create custom analysis patterns.
              </p>
            </div>
          </div>
