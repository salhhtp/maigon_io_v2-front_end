...
export function tokenVariants(token) {
    ...
    const fallbackKey = Object.keys(STRUCTURAL_SYNONYMS).find((key) => STRUCTURAL_SYNONYMS[key]?.includes(token));
    if (fallbackKey) {
...
export function isStructuralToken(token) {
    if (STRUCTURAL_TOKENS.has(token))
        return true;
    return Object.keys(STRUCTURAL_SYNONYMS).some((key) => tokenVariants(key).includes(token));
}
function requiredStructuralHits(structuralTokens) {
    if (structuralTokens.length <= 1)
        return 1;
    return 2;
}
function countStructuralMatches(structuralTokens, clauseTokens) {
    if (!structuralTokens.length || !clauseTokens.length)
        return 0;
...
export function hasStructuralMatch(structuralTokens, clauseTokens) {
    if (!structuralTokens.length)
        return true;
...
function tokenMatchesClause(token, clauseTokenSet) {
...
function countTokenMatches(requirementTokens, clauseTokens) {
    if (!requirementTokens.length || !clauseTokens.length)
        return 0;
    const clauseSet = new Set(clauseTokens);
    let hits = 0;
    requirementTokens.forEach((token) => {
        if (tokenMatchesClause(token, clauseSet))
            hits += 1;
    });
    return hits;
}
function coverageWithSynonyms(requirementTokens, clauseTokens) {
...
function queryCoverageScore(queryTokens, candidateTokens) {
...
function buildNgrams(value, n = 4) {
...
function scoreTextSimilarity(query, candidate) {
    ...
    const tokenScore = Math.max(jaccardSimilarity(tokensQuery, tokensCandidate), queryCoverageScore(tokensQuery, tokensCandidate));
...
function getClauseIdentifier(clause) {
    const candidate = typeof clause.clauseId === "string"
        ? clause.clauseId
...
function normalizeClauseId(value) {
    const normalized = normalizeForMatch(value ?? "");
    if (!normalized)
        return "";
    return normalized.replace(/\s+/g, "-");
}
function rankClauseCandidates(query, clauses, mode) {
    const results = [];
    for (const clause of clauses) {
        const clauseId = getClauseIdentifier(clause) ?? "";
        if (!clauseId)
            continue;
        const clauseText = mode === "heading"
            ? clause.title ?? ""
...
function resolveClauseById(clauseId, clauses) {
    const normalized = normalizeClauseId(clauseId);
    if (!normalized)
        return null;
    return (clauses.find((clause) => normalizeClauseId(getClauseIdentifier(clause)) === normalized) ?? null);
}
export function matchClauseToText(text, clauses) {
...
export function resolveClauseMatch(options) {
    ...
    const headingQuery = typeof clauseReference?.heading === "string"
        ? clauseReference?.heading
        : null;
    const excerptQueryRaw = typeof clauseReference?.excerpt === "string"
        ? clauseReference?.excerpt
        : null;
    const excerptQuery = excerptQueryRaw && !isMissingEvidenceMarker(excerptQueryRaw)
        ? excerptQueryRaw
        : null;
    const fallbackQuery = typeof fallbackText === "string" ? fallbackText : null;
    ...
    const prefersDefinitionHeading = Boolean(headingQuery &&
        topHeading &&
        normalizeForMatch(headingQuery).includes("definition") &&
        normalizeForMatch(topHeading.title ?? "").includes("confidential information"));
    const shouldPreferHeading = topHeading &&
    ...
    const bestClause = clauses.find((clause) => normalizeClauseId(getClauseIdentifier(clause)) ===
        normalizeClauseId(preferredCandidate.clauseId)) ?? null;
    ...
    if (shouldPreferHeading && topHeading) {
        const hasHeading = topCandidates.some((candidate) => normalizeClauseId(candidate.clauseId) ===
...
export function isMissingEvidenceMarker(value) {
    if (!value)
        return false;
    const normalized = value.trim().toLowerCase();
    return MISSING_EVIDENCE_MARKERS.some((marker) => normalized.includes(marker));
}
export function buildEvidenceExcerpt(options) {
    ...
    if (clauseText.length <= maxLength)
        return clauseText;
    const anchorText = typeof options.anchorText === "string" ? options.anchorText.trim() : "";
    if (anchorText) {
 ...
 */
const CRITERION_ANCHOR_PHRASES = {
...
export function buildUniqueEvidenceExcerpt(options) {
    ...
    if (clauseText.length <= maxLength)
        return clauseText;
    const excludeExcerpts = options.excludeExcerpts ?? new Set();
    const clauseLower = clauseText.toLowerCase();
    // Get criterion-specific phrases if available
    const criterionId = options.criterionId ?? "";
    const criterionKey = Object.keys(CRITERION_ANCHOR_PHRASES).find((key) => criterionId.toUpperCase().includes(key));
...
function normalizeAnchorText(anchorText, clauseText, maxLength = 160) {
    const trimmed = anchorText.trim();
    const clauseSource = clauseText ?? "";
    if (!clauseSource.trim())
        return trimmed;
    const candidates = [];
    if (trimmed)
...
export function checkEvidenceMatch(excerpt, content) {
...
export function checkEvidenceMatchAgainstClause(excerpt, clauseText) {
...
function scoreAnchorToIssue(anchor, issue) {
    const candidate = [
...
function scoreEditToIssue(edit, issue) {
    const editParts = [
        edit.id,
        edit.intent,
        edit.rationale,
    ...
    if (edit.id && Array.isArray(issue.tags)) {
        const tagMatch = issue.tags.some((tag) => normalizeForMatch(tag) === normalizeForMatch(edit.id ?? ""));
        if (tagMatch)
...
function isPlaceholderEdit(edit) {
    const proposedText = typeof edit.proposedText === "string" ? edit.proposedText : "";
    return isPlaceholderText(proposedText);
}
function getEditContent(edit) {
    if (typeof edit.proposedText === "string" && edit.proposedText.trim()) {
        return edit.proposedText.trim();
    }
    const updatedText = typeof edit.updatedText === "string" ? edit.updatedText.trim() : "";
    if (updatedText)
        return updatedText;
...
function extractEditHeadingHint(text) {
...
function clauseHasDefinitionSignals(value) {
    const normalized = normalizeForMatch(value);
    if (!normalized.includes("confidential information"))
        return false;
    if (normalized.includes("shall mean"))
        return true;
    if (normalized.includes("definition"))
        return true;
    if (normalized.includes("means"))
        return true;
    return normalized.includes("shall not include") || normalized.includes("does not include");
}
function clauseHasReceivingPartyObligation(value) {
    const normalized = normalizeForMatch(value);
    return (normalized.includes("receiving party shall") ||
...
function clauseSupportsUseLimitation(clause, clauseText) {
    const normalized = normalizeForMatch(`${clause.title ?? ""} ${clauseText}`);
    const hasObligation = clauseHasReceivingPartyObligation(normalized);
    const hasUsePurpose = normalized.includes("purpose") ||
...
function clauseSupportsReturnDestruction(clause, clauseText) {
    const normalized = normalizeForMatch(`${clause.title ?? ""} ${clauseText}`);
    const hasConfidential = normalized.includes("confidential information");
    const hasReturn = normalized.includes("return");
    const hasDestroy = normalized.includes("destroy") || normalized.includes("destruction");
    return hasConfidential && (hasReturn || hasDestroy);
}
function clauseSupportsTermSurvival(clause, clauseText) {
    ...
    const clauseTokens = tokenizeForMatch(`${clause.title ?? ""} ${clauseText}`);
    const hasTermSignals = clauseTokens.includes("term") ||
        clauseTokens.includes("duration") ||
        clauseTokens.includes("expires") ||
        clauseTokens.includes("expiration");
    const hasSurvivalSignals = clauseTokens.includes("survival") || clauseTokens.includes("survive");
    return hasTermSignals || (hasSurvivalSignals && clauseTokens.includes("agreement"));
}
function clauseSupportsRemediesEdit(clause, clauseText) {
    const normalized = normalizeForMatch(`${clause.title ?? ""} ${clauseText}`);
...
function clauseSupportsIpEdit(clause, clauseText) {
    const tokens = tokenizeForMatch(`${clause.title ?? ""} ${clauseText}`);
    const tokenSet = new Set(tokens);
    const hasLicense = tokenSet.has("license") ||
        tokenSet.has("licence") ||
        tokenSet.has("licensed") ||
        tokenSet.has("licensing");
    const hasIp = tokenSet.has("ip") ||
...
function clauseSupportsCompelledDisclosure(clause, clauseText) {
    const normalized = normalizeForMatch(`${clause.title ?? ""} ${clauseText}`);
    if (normalized.includes("required by law"))
        return true;
    if (normalized.includes("required to disclose"))
        return true;
    if (normalized.includes("court order"))
        return true;
...
function getEditSupportCheck(editContent) {
    ...
    if (normalized.includes("confidential information") &&
        ...
        return (clause, clauseText) => clauseHasDefinitionSignals(`${clause.title ?? ""} ${clauseText}`);
    }
    if (normalized.includes("injunctive") ||
    ...
    if (normalized.includes("need to know") ||
    ...
    if (normalized.includes("compelled") ||
...
function findBestSupportedClause(editContent, clauses, supportCheck) {
    let best = null;
    clauses.forEach((clause) => {
        const clauseText = clause.originalText ?? clause.normalizedText ?? clause.title ?? "";
        if (!clauseText)
            return;
        if (!supportCheck(clause, clauseText))
            return;
...
function hasCompelledDisclosureProcessSignals(value) {
    const normalized = normalizeForMatch(value);
    if (!normalized)
        return false;
    const hasTrigger = normalized.includes("required by law") ||
        normalized.includes("court order") ||
    ...
    const hasNotice = normalized.includes("notice") ||
        normalized.includes("notify") ||
        normalized.includes("notification") ||
        normalized.includes("prompt");
    const hasProtection = normalized.includes("protective order") ||
 ...
 */
function isRedundantEdit(edit, clauses) {
    ...
    if (!supportCheck)
        return false;
    const supportedMatch = findBestSupportedClause(fullContent, clauses, supportCheck);
    if (!supportedMatch)
        return false;
    const clauseText = supportedMatch.clause.originalText ??
    ...
    if (supportCheck === clauseSupportsCompelledDisclosure &&
    ...
    const structuralTokens = editTokens.filter((token) => isStructuralToken(token));
    const hasStructuralOverlap = structuralTokens.length === 0 ||
        hasStructuralMatch(structuralTokens, clauseTokens);
    const structuralCoverage = structuralTokens.length > 0
...
export function bindProposedEditsToClauses(options) {
    ...
    issues.forEach((issue) => {
        ...
        const hasMissingEvidence = clauseId.startsWith("missing") ||
    ...
    const filteredEdits = proposedEdits.filter((edit) => edit &&
        typeof edit === "object" &&
    ...
    issues.forEach((issue) => {
        ...
        if (normalizedId && clauseIds.has(normalizedId)) {
            issueClauseMap.set(issue, clauseIdMap.get(normalizedId) ?? clauseId);
            return;
        }
        ...
        if (normalizedMatched && clauseIds.has(normalizedMatched)) {
            issueClauseMap.set(issue, clauseIdMap.get(normalizedMatched) ?? matchedId ?? "");
        }
    });
    return filteredEdits.map((edit, index) => {
        if (!edit || typeof edit !== "object")
            return edit;
        const rawAnchorText = typeof edit.anchorText === "string" ? edit.anchorText.trim() : "";
        const rawPreviousText = typeof edit.previousText === "string" ? edit.previousText.trim() : "";
        const intent = typeof edit.intent === "string" ? edit.intent.toLowerCase() : "";
        const missingAnchor = isMissingEvidenceMarker(rawPreviousText || rawAnchorText);
        const editId = typeof edit.id === "string" ? edit.id : "";
        const normalizedEditId = editId.toLowerCase();
        const editClauseId = typeof edit.clauseId === "string" ? edit.clauseId : "";
        const normalizedEditClauseId = normalizeClauseId(editClauseId);
        const isAutoMissingEdit = normalizedEditId.startsWith("auto-") ||
        ...
        const editStructuralTokens = editTokens.filter((token) => isStructuralToken(token));
        const currentId = typeof edit.clauseId === "string" ? edit.clauseId.trim() : "";
        ...
        if (normalizedCurrentId && clauseIds.has(normalizedCurrentId)) {
            ...
            const structuralMismatch = editStructuralTokens.length > 0 &&
                !hasStructuralMatch(editStructuralTokens, clauseTokens);
            const anchorText = typeof edit.anchorText === "string" ? edit.anchorText : "";
            ...
            if (structuralMismatch)
                nextIntent = "insert";
            if (nextIntent === "replace" && !anchorMatches)
                nextIntent = "insert";
            ...
            if (editTokens.length >= 3 && rebindQuery.trim()) {
                ...
                const altId = altMatch.match
                    ...
                    : null;
                const normalizedAlt = normalizeClauseId(altId);
                const canRebindByText = normalizedAlt &&
                    normalizedAlt !== normalizedCurrentId &&
                    altMatch.confidence >=
                ...
                if (canRebindByText) {
                    ...
                    const altText = altClause
                        ? altClause.originalText ?? altClause.normalizedText ?? altClause.title ?? ""
                        : "";
                    const rebindAnchorSeed = editContent || rebindQuery;
                    const altAnchor = altText
            ...
            if (headingHint) {
                ...
                const hintThreshold = headingHint.strength === "strong" ? MIN_HEADING_SCORE : STRONG_HEADING_SCORE;
                if (normalizedHint &&
            ...
            if (supportCheck && boundClause) {
                ...
                if (!supports) {
                    const supported = findBestSupportedClause(editContent || editSignal, clauses, supportCheck);
                    ...
                    if (supported &&
                        ...
                        const supportedText = supportedClause.originalText ??
        ...
        let bestEditIssue = null;
        let bestEditScore = 0;
        let bestAnchorIssue = null;
        ...
        const useEditMatch = bestEditIssue !== null && bestEditScore >= MIN_MATCH_SCORE;
        const useAnchorMatch = !useEditMatch &&
        ...
        if (bestIssue && bestScore >= MIN_MATCH_SCORE) {
            const bestIssueClauseId = normalizeClauseId(bestIssue.clauseReference?.clauseId ?? "");
            const bestIssueMissing = bestIssueClauseId.startsWith("missing") ||
            ...
            if (boundId) {
                ...
                const clauseText = boundClause
                    ...
                    : "";
                const clauseTokens = tokenizeForMatch(clauseText);
                const issueStructuralMismatch = editStructuralTokens.length > 0 &&
                    !hasStructuralMatch(editStructuralTokens, clauseTokens);
                const issueAnchor = bestIssue.clauseReference?.excerpt ??
                ...
                if (!issueStructuralMismatch || anchorMatches) {
                    const boundMissing = isMissingEvidenceMarker(bestIssue.clauseReference?.excerpt) ||
        ...
        if (fallbackQuery) {
            ...
            if (normalizedMatched && clauseIds.has(normalizedMatched)) {
                ...
                const nextIntent = edit.intent === "replace" && !anchorMatches ? "insert" : edit.intent;
...
export function assessEditSemanticDrift(options) {
    ...
    if (!edits.length || !clauses.length)
        return [];
    const clauseById = new Map();
    clauses.forEach((clause) => {
        const clauseId = getClauseIdentifier(clause);
        const normalized = normalizeClauseId(clauseId);
        if (!normalized || clauseById.has(normalized))
            return;
        clauseById.set(normalized, clause);
    });
    const alerts = [];
    edits.forEach((edit, index) => {
        if (!edit || typeof edit !== "object")
            return;
        const intent = typeof edit.intent === "string" ? edit.intent.toLowerCase() : "";
        if (intent.includes("insert") || intent.includes("remove"))
            return;
        const proposedText = typeof edit.proposedText === "string" ? edit.proposedText.trim() : "";
        if (proposedText.length < 60)
            return;
        const anchorSeed = typeof edit.previousText === "string" && edit.previousText.trim().length > 0
            ? edit.previousText.trim()
            : typeof edit.anchorText === "string"
                ? edit.anchorText.trim()
                : "";
        if (isMissingEvidenceMarker(anchorSeed))
            return;
        const rawClauseId = typeof edit.clauseId === "string" ? edit.clauseId.trim() : "";
        ...
        if (!clause)
            return;
        const clauseText = clause.originalText ?? clause.normalizedText ?? clause.title ?? "";
        if (clauseText.trim().length < 60)
            return;
        const similarity = scoreTextSimilarity(proposedText, clauseText).score;
        const clauseTokens = tokenizeForMatch(clauseText);
        const proposedTokens = tokenizeForMatch(proposedText);
        const clauseStructural = clauseTokens.filter((token) => isStructuralToken(token));
        const proposedStructural = proposedTokens.filter((token) => isStructuralToken(token));
        const structuralMismatch = clauseStructural.length >= 2 &&
            proposedStructural.length >= 2 &&
            !hasStructuralMatch(clauseStructural, proposedStructural);
        const minSimilarity = options.minSimilarity ?? (structuralMismatch ? 0.32 : 0.24);
        if (similarity >= minSimilarity)
            return;
        alerts.push({
            editId: typeof edit.id === "string" && edit.id.trim().length > 0
                ? edit.id
                : `edit-${index + 1}`,
...
export function findRequirementMatch(requirement, clauses, content) {
    if (!requirement || !requirement.trim()) {
        return { met: false, score: 0 };
    }
    const normalizedRequirement = normalizeForMatch(requirement);
    const requiresCompelledSignals = normalizedRequirement.includes("compelled") ||
    ...
    const requiresUseLimitationSignals = normalizedRequirement.includes("use limitation") ||
        normalizedRequirement.includes("need to know") ||
        normalizedRequirement.includes("need-to-know") ||
        (normalizedRequirement.includes("purpose") &&
            normalizedRequirement.includes("use"));
    const requiresDefinitionSignals = normalizedRequirement.includes("definition") &&
        normalizedRequirement.includes("confidential");
    const requiresIpLicenseSignals = normalizedRequirement.includes("license") &&
    ...
    const hasCompelledDisclosureSignals = (tokens) => {
        if (!tokens.length)
            return false;
        const tokenSet = new Set(tokens);
    ...
    const hasStrongDefinitionSignals = (value) => {
        const normalized = normalizeForMatch(value);
        if (!normalized.includes("confidential information"))
            return false;
        if (normalized.includes("shall mean"))
    ...
    const hasIpLicenseSignals = (tokens) => {
        if (!tokens.length)
            return false;
        const tokenSet = new Set(tokens);
    ...
    if (requiresIpLicenseSignals) {
        for (const clause of clauses) {
            const clauseText = `${clause.title ?? ""} ${clause.originalText ?? ""} ${clause.normalizedText ?? ""}`;
            const clauseTokens = tokenizeForMatch(clauseText);
    ...
    const requiresHeadingMatch = requirementTokens.length === 1 &&
        requirementTokens.some((token) => HEADING_PRIORITY_TOKENS.has(token));
    const structuralTokens = requirementTokens.filter((token) => isStructuralToken(token));
    const matchTokens = structuralTokens.length > 0 ? structuralTokens : requirementTokens;
    const minCoverage = matchTokens.length <= 2
        ? 0.75
    ...
    for (const clause of clauses) {
        ...
        const shouldPromote = score > bestScore ||
            (score === bestScore &&
    ...
    if (topHeading && topHeading.score >= MIN_HEADING_SCORE) {
        const headingClause = clauses.find((entry) => normalizeClauseId(getClauseIdentifier(entry)) ===
            normalizeClauseId(topHeading.clauseId)) ?? null;
        if (headingClause) {
            const clauseText = `${headingClause.title ?? ""} ${headingClause.originalText ?? ""} ${headingClause.normalizedText ?? ""}`;
            const clauseTokens = tokenizeForMatch(clauseText);
            if (!requiresCompelledSignals || hasCompelledDisclosureSignals(clauseTokens)) {
                const meetsUseLimitation = !requiresUseLimitationSignals ||
                    clauseSupportsUseLimitation(headingClause, clauseText);
                const meetsDefinitionSignals = !requiresDefinitionSignals || hasStrongDefinitionSignals(clauseText);
                const passesStructure = structuralTokens.length === 0 ||
                    hasStructuralMatch(structuralTokens, clauseTokens);
                const preferHeading = requirementTokens.some((token) => HEADING_PRIORITY_TOKENS.has(token));
                const preferDefinitionHeading = requirementTokens.includes("definition") &&
                    normalizeForMatch(headingClause.title ?? "").includes("confidential information");
                const headingThreshold = preferHeading
                    ? MIN_HEADING_SCORE
                    : bestScore + 0.08;
                if (meetsUseLimitation &&
...
function isOptionalAnchor(anchor) {
    return /\bif (applicable|relevant)\b/i.test(anchor);
}
export function evaluatePlaybookCoverageFromContent(playbook, options) {
    ...
    const criticalClauses = (playbook.criticalClauses ?? []).map((clause) => {
        ...
        const missingMustInclude = clause.mustInclude?.filter((item) => {
    ...
    const requiredAnchors = anchorCoverage.filter((anchor) => !isOptionalAnchor(anchor.anchor));
    const totalChecks = criticalClauses.length + requiredAnchors.length || 1;
    const metChecks = criticalClauses.filter((clause) => clause.met).length +
...
export function buildRetrievedClauseContext(options) {
    ...
    for (const anchor of anchors) {
        ...
        const anchorStructuralTokens = anchorTokens.filter((token) => isStructuralToken(token));
        const forceHeading = anchorTokens.some((token) => HEADING_PRIORITY_TOKENS.has(token)) &&
            topHeading &&
            topHeading.score >= MIN_HEADING_SCORE;
        const preferHeading = topHeading &&
            topHeading.score >= MIN_HEADING_SCORE &&
        ...
        if (forceHeading || preferHeading) {
            if (topHeading) {
                const alreadyIncluded = candidates.some((candidate) => normalizeClauseId(candidate.clauseId) ===
                    normalizeClauseId(topHeading.clauseId));
                if (!alreadyIncluded) {
                    ...
                    candidates = candidates.sort((a, b) => {
                        if (normalizeClauseId(a.clauseId) ===
                            normalizeClauseId(topHeading.clauseId)) {
                            return -1;
                        }
                        if (normalizeClauseId(b.clauseId) ===
                            normalizeClauseId(topHeading.clauseId)) {
                            return 1;
                        }
        ...
        if (forceHeading && topHeading) {
            candidates = candidates.filter((candidate) => normalizeClauseId(candidate.clauseId) ===
                normalizeClauseId(topHeading.clauseId));
        }
        ...
        const canUseClause = (clauseText, clauseTokens) => {
            if (minClauseLength > 0 && clauseText.length < minClauseLength) {
                return false;
            }
            if (anchorStructuralTokens.length > 0 &&
        ...
        for (const candidate of candidates) {
            if (snippets.length >= maxTotal)
                break;
            const seenKey = `${anchor}::${candidate.clauseId}`;
            if (seen.has(seenKey))
                continue;
            const clause = clauses.find((entry) => normalizeClauseId(getClauseIdentifier(entry)) ===
            ...
            seen.add(seenKey);
            addedForAnchor += 1;
            if (addedForAnchor >= maxPerAnchor)
                break;
        }
...
export function dedupeIssues(issues) {
    ...
    issues.forEach((issue) => {
        ...
        const missingClause = isMissingClauseId(clauseId) ||
            isMissingEvidenceMarker(issue.clauseReference?.excerpt);
        let bucketKey = normalizeClauseId(clauseId) || clauseId;
        if (missingClause) {
            const keySource = issue.id ??
                issue.title ??
                issue.category ??
                issue.clauseReference?.heading ??
                "";
    ...
    buckets.forEach((bucket) => {
        ...
        bucket.forEach((issue) => {
            ...
            const matchIndex = kept.findIndex((existing) => {
                const existingText = `${existing.title ?? ""} ${existing.recommendation ?? ""}`.trim();
            ...
            const existingRank = severityRank[(existing.severity ?? "").toLowerCase()] ?? 0;
            const nextRank = severityRank[(issue.severity ?? "").toLowerCase()] ?? 0;
...
export function dedupeProposedEdits(edits) {
    ...
    editsWithSort.forEach(({ edit }) => {
        ...
        const matchIndex = deduped.findIndex((existing) => {
            const existingText = `${existing.intent ?? ""} ${existing.proposedText ?? ""}`.trim();
...
export function normaliseReportExpiry(value) {
    if (typeof value !== "string") {
        return new Date(Date.now() + 86400000 * 30).toISOString();
    }
