...
async function createAdminUser(supabase: any, request: CreateAdminRequest) {
  try {
    ...
    const existingAuthUser = existingAuthUsers?.users?.find((user: any) => user.email === request.email)
      ...
      adminUsers.map(async (user) => {
        ...
        if (user.auth_user_id) {
          try {
            ...
            console.warn(`Could not fetch auth info for user ${user.email}:`, error)
          }
        }
...
async function deleteAdminUser(supabase: any, email: string) {
  try {
    ...
    if (profile.auth_user_id) {
      ...
      if (deleteError) {
        return new Response(
          JSON.stringify({ error: 'Failed to delete user profile', details: deleteError.message }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
    }
