...
import { CONTRACT_PLAYBOOKS, type PlaybookKey } from "../shared/ai/playbooks";
import {
...
type DebugReport = {
  meta: {
    file: string;
    extractor: Extractor;
    extractorRequested: Extractor;
    playbookKey: PlaybookKey;
    playbookTitle: string;
    contentLength: number;
    clauseCount: number;
  };
  clauseDigest: ClauseDigest;
  retrievedExcerpts: ReturnType<typeof buildRetrievedClauseContext>;
  coverage: ReturnType<typeof evaluatePlaybookCoverageFromContent>;
...
function resolvePlaybook(value?: string): PlaybookKey {
  if (!value) return "non_disclosure_agreement";
  const normalized = value.toLowerCase().replace(/[^a-z0-9]+/g, "_");
  const aliasMap: Record<string, PlaybookKey> = {
  ...
  if ((CONTRACT_PLAYBOOKS as Record<string, unknown>)[normalized]) {
    return normalized as PlaybookKey;
  }
...
function buildClauses(
  ...
  return segments.map((segment, index) => {
    const clauseId =
      segment.heading.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 64) ||
      `clause-${index + 1}`;
...
function buildClauseDigest(clauses: ClauseExtraction[]): ClauseDigest {
  ...
  if (extension === ".docx") {
    if (extractorRequested === "mammoth") {
      ...
      if (!content || content.trim().length === 0) {
        console.warn(
          "[debug-review] Edge-style DOCX extraction returned empty text; falling back to mammoth.",
        );
        content = await extractDocxTextMammoth(buffer);
