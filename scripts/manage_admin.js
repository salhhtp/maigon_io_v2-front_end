...
async function createAdminUser() {
  ...
  try {
    ...
    if (response.status === 201) {
      ...
      console.log(
        `🔗 Try signing in at: ${SUPABASE_URL.replace("supabase.co", "supabase.co").replace("https://", "https://").replace(".supabase.co", "")}/signin`,
      );
    ...
    console.error("\nTroubleshooting:");
    console.error(
      "1. Make sure the admin-user-management function is deployed",
    );
    console.error(
      "2. Check that ADMIN_MANAGEMENT_KEY matches in Supabase secrets",
    );
    console.error(
      "3. Verify SUPABASE_SERVICE_ROLE_KEY is set in Supabase secrets",
    );
  }
}
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      users.forEach((user, index) => {
        ...
        if (!user.can_sign_in) {
          console.log(
            `   ⚠️  Status: Cannot sign in - auth user not properly created`,
          );
        } else {
    ...
    console.error(
      "2. Check that ADMIN_MANAGEMENT_KEY is set in Supabase secrets",
    );
